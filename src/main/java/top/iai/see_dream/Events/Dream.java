...
public class Dream {
    ...
    private void Dreaming() {
        //获取第一个玩家
        if (world.playerEntities.get(0) != null){
        ...
        //在范围内取随机数
        int randomX = (int) player.posX - rangeX + random.nextInt(2 * rangeX + 1);
        int randomY = (int) (player.posY + player.getEyeHeight()) - rangeY + random.nextInt(2 * rangeY + 1);
        int randomZ = (int) player.posZ - rangeZ + random.nextInt(2 * rangeZ + 1);
        ...
        if (result != null && result.typeOfHit == RayTraceResult.Type.BLOCK) {
            ...
            if (possibleTransformations != null && !possibleTransformations.isEmpty()) {
                ...
                if (currentBlock == Blocks.GRASS || possibleTransformations == plantsBlocks){
                    ...
                    world.setBlockState(hitPos.add(random.nextInt(3)-1, random.nextInt(2),random.nextInt(3)-1), newBlock.getDefaultState(), 3);
                    return;
                }
                newBlock = possibleTransformations.get(random.nextInt(possibleTransformations.size()));
            }
            ...
            // 替换方块
            world.setBlockState(hitPos, newBlock.getDefaultState(), 3);
        }
    }
